...
training_args = TrainingArguments(
    ...
    per_device_train_batch_size=1, #通俗解释： 这个超级大脑一次能吃下多少条训练数据。这里设定为一次看2条。
    ...
    max_steps=200, # 默认1000， 整个训练计划总共要训练多少步。训练完1000步就自动结束
    # gradient_accumulation_steps=2,
    # 通俗解释： “存钱罐”策略。因为一次只处理2条数据（上面设置的），可能太少了，学习方向不稳定。那就让它先偷偷地内部模拟学习16次（每次2条），把这16次学习的经验攒起来，然后再一次性真正地更新大脑
    #w armup_steps=1,  # 训练前的“热身运动”。在最开始的50步训练里，学习率会从0慢慢地、线性地增加到你设定的初始值（2e-4）
    #max_steps=2,
    learning_rate=2e-4, #学习的速度，或者说每次调整的“步伐”大小。2e-4 就是 0.0002。
    logging_dir="logs",
    logging_steps=10, #每训练多少步，就记录一次日志。这里设定为每10步记录一次。
    # learning_rate=5e-5,   # 降低学习率，避免 NaN
    # #warmup_steps=30,      # 或者 warmup_ratio=0.1
    # warmup_ratio = 0.1,
    # num_train_epochs=3,   # 比 max_steps=100 更合理
